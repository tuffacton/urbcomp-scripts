...
def plfit(x, *varargin):
    ...
    while i<len(varargin): 
        ...
        if not argok:
            print('(PLFIT) Ignoring invalid argument #{}'.format(i+1))
    ...
    if     all(floor(Y)==float(Y) for Y in x): f_dattype = 'INTS'
    elif not all((floor(Y)==float(Y)) for Y in x):    f_dattype = 'REAL'
    else:                 f_dattype = 'UNKN'
